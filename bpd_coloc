...
#mysterious bug fix - issue with two sample MR install 
...
# '/exports/igmm/eddie/GenScotDepression/users/Laurence/sumstats_BPD_bp/Edited2_PGC3_BPD_wave3.european.autosome.public.v3.vcf.tsv'
...
coloc_full_output = as.data.frame(do.call(rbind, coloc_full_output))
coloc_full_output = coloc_full_output[rev(order(coloc_full_output$PP.H4)), ]

write.csv(coloc_full_output, "/exports/igmm/eddie/GenScotDepression/users/Laurence/coloc_output/BPD_coloc1.csv", row.names = F)
