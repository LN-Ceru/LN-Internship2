...
PRS_0.05 =  read.csv("./SCZ_prs0.05.score", header = TRUE, sep = '')
covars = read.csv("./GS_covar_mediumrare_10.txt", header = TRUE, sep = '')

#merging on the PRS for MDD first

norm_prot_prs = merge(norm_prot_read, PRS_0.05, by.x = 'IID', by.y = 'IID')
...
norm_prot_results <- data.frame(SeqId = 1:length, sczPRS_beta = 1:length, sczPRS_SE = 1:length, sczPRS_P = 1:length)



#single protein test
#prot_name <- as.character(colnames(markers[,1]))
#norm_mod <- lmekin(scale(norm_prot_id[,prot_name]) ~ scale(norm_prot_id$Pt_0.05) + as.factor(norm_prot_id$sex) + scale(norm_prot_id$age) + (1|norm_prot_id$IID), data = norm_prot_id, varlist = kin_model*2)
...
write.csv(norm_prot_results, "/exports/igmm/eddie/GenScotDepression/users/Laurence/assoc_output/SCZ_mediumrare_prot_res_0.05.csv", row.names = F, quote = F)
